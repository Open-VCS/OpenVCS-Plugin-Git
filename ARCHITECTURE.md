...
bootstrap. The authored Git module lives in `src/plugin.ts` and compiles to
`bin/plugin.js`, where `PluginDefinition` declares the Git `vcs.*` delegates and
runtime options while `OnPluginStart()` remains available for startup work.
